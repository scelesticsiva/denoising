...
class BaseDatasetCARE(Dataset):
    def __init__(self,
                 npz_fpth: str,
                 ):
        super().__init__()
        self.npz_fpth = npz_fpth
        self.images = np.transpose(np.load(npz_fpth)['Y'], (0, 2, 3, 1))
        print(f'Shape of the dataset: {self.images.shape}')

        self.transform = get_transforms('care', None, None)
